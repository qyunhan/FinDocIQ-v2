...
order, and what each item buys**. Generated from `db/compiled_v2.db` +
`data/derived/dashboards/` on 2026-08-14 (regenerated after the
double-count correction in Appendix E). Regenerate rather than hand-edit.

| | stamped | total |
|---|---|---|
| `row_dim.canonical_leaf_id` | 3,804 | 5,771 (68% excluding Pillar 3) |
| `col_dim.canonical_col_id` | 210 | 1,915 |
| declared anchors captured | 166 | 166 |

**Tier 1 — blank dashboard cells: NONE.** All three were cleared on
2026-08-14; every declared anchor now resolves. This tier is the one to
re-check first after any anchor or masterlist edit.

**Tier 2 — 255 rows carrying figures in tables we already classify.**
The table has a `table_type_id`, the row prints a label and a number, and it
still has no leaf id. Cheapest row-side wins: the work is adding leaves to an
existing block, not deciding what a table is.

| Rows | Bank | table_type_id |
|---|---|---|
| 52 | OCBC | `FS_BALANCE_SELECTED` |
...
**Tier 3 — 13 `(bank, table_type_id)` pairs with stampable columns and no
column block.** No dashboard anchor addresses these today, so the payoff is
unblocking the segment/equity sections Appendix C marks `✱ col_dim
canonicalisation blocks authoring`.

| Cells | Cols | Bank | table_type_id |
|---|---|---|---|
...
**Tier 4 — 63 tables never classified** (54 captions, Pillar 3 held
out). Largest volume, lowest payoff. Statement of Changes in Equity leads it
and also appears in Tier 3 — the column block must come first or the row work
has nowhere to land.

| Rows | Tables | Bank | Printed caption |
|---|---|---|---|
| 40 | 1 | OCBC | STATEMENT OF CHANGES IN EQUITY - GROUP For the financial y |
| 40 | 1 | OCBC | STATEMENT OF CHANGES IN EQUITY - GROUP For the financial y |
| 38 | 1 | OCBC | UNAUDITED STATEMENT OF CHANGES IN EQUITY - GROUP For the h |
| 37 | 1 | DBS | CUSTOMER DEPOSITS |
| 30 | 1 | DBS | PERFORMANCE BY BUSINESS SEGMENTS — Selected income stateme |
| 30 | 3 | OCBC | Movements in Level 3 non-financial assets |
| 26 | 1 | OCBC | UNAUDITED CONSOLIDATED STATEMENT OF COMPREHENSIVE INCOME F |
| 25 | 1 | OCBC | 13.2 Geographical segments |
| 22 | 2 | OCBC | Movements in Level 3 financial liabilities |
| 21 | 1 | UOB | 6 Fair Value of Financial Instruments (b) The Group classi |

**Out of scope:** Pillar 3 —
`DBS_1Q26_P3_other_regulatory_disclosures`, 197 rows, 0 stamped, no
masterlist and no dashboard addressing it. Quote the ex-Pillar-3 coverage
figure when reporting.

---
## Appendix E — The replay divergence, and the double-count it exposed

**Read this before trusting any committed database.** It is the sharpest lesson
the project has produced: *every count matched, and the numbers were still wrong.*

### What was believed

`findociq/db/compiled_fs.db` is committed, and `--rebuild-db --only
4Q25,1Q26,2Q26` reconstructs it from the cached extraction artifacts. A rebuild
reproduces the headline figures exactly — 10 docs, 488 sections, 342 tables,
5,771 rows, 21,581 cells, verify PASS — so the committed artifact and a fresh
replay were assumed interchangeable. `TO_FIX` §5 had flagged that they were not,
but framed the difference as the replay LOSING work: it stamped 39 fewer
`canonical_leaf_id`s (3,843 → 3,804). On that reading the committed DB was the
better artifact and a rebuild was a downgrade.

### What is actually true

The rebuild is **deterministic** — two independent runs produced byte-identical
row identity and table classification (0 differences). So the divergence was
never run-to-run randomness; the committed artifact simply carried stamping that
today's classifier no longer produces.

All 39 extra stamps sat in ONE document,
`OCBC_4Q25_Media_Release_and_Financial_Highlights`, across three tables of 13
rows each. That document is one of the **duplicate-extraction** cases: the same
physical page is extracted as several `table_t` rows with identical content, and
`dedup_status` is NULL on all of them, so the quarantine pass never caught them.
In the committed DB one duplicate had been classified `FS_INCOME_SELECTED`; the
current arbiter classifies it `FS_BALANCE_SELECTED` like its siblings.

**Those 39 stamps were not an asset. They were a double count.** With the same
income leaves stamped on two copies of one page, the anchor composition — which
SUMS its members — added each figure twice. Measured on the live dashboard:

| OCBC loans breakdown @ 31-Dec-25 | committed DB | after rebuild | filed |
|---|---|---|---|
| Singapore | 298,172 | **149,086** | |
| Greater China | 141,898 | **70,949** | |
| Asia | 141,354 | **70,677** | |
| Rest of World | 100,816 | **50,408** | |
| **sum** | **682,240** | **341,120** | **341,120** |

The rebuilt figures reconcile to the filed gross loans exactly, and so do the
industry, maturity and business-unit blocks — each sums to 341,120. The
committed DB was serving exactly double on six cells of the live dashboard.

The rebuild was **corrective, not lossy**, and the replay was promoted to be the
committed artifact on 2026-08-14.

### Why every check passed

Nothing in the pipeline was positioned to catch it:

- **Row, table and cell counts were identical** — the duplicate rows exist in
  both artifacts; only their `table_type_id` differed.
- **`verify_cells` passed 10/10** — it checks each value against its page, and
  each value WAS on its page. Verification is per-cell; double counting is a
  property of composition.
- **Anchor coverage was 166/166 both ways** — an address resolving twice still
  resolves.
- **The number was plausible.** 298,172 for Singapore is not obviously absurd
  until you add the block up.

The check that would have caught it is the one now written down: **a breakdown
must sum to its own total.** Geography, industry, maturity and business unit each
have a declared total in the same exhibit; comparing the parts against it is a
cheap, general invariant that needs no masterlist knowledge.

### Rules this produces

1. **Counts are not evidence of equivalence.** Compare identity — the
   `(doc, table, row) -> canonical_leaf_id` set — not row totals.
2. **A committed artifact is not automatically more correct than a rebuild.** It
   is older, and age accumulates hand-patches as readily as it accumulates
   value. Both must be diffed on identity before one is preferred.
3. **Rebuild to a scratch path.** `--rebuild-db --db <tmp>` has always been
   supported; overwriting the shipped DB to test a hypothesis destroyed it once
   during this investigation, and only a pre-run copy saved it.
4. **Duplicate extraction is the upstream defect.** It caused the double count
   here and the split stamping on OCBC's 2Q26 p.9 highlights page. Until
   `dedup_status` reliably marks these clusters, any exhibit extracted more than
   once is a live double-count risk wherever an anchor sums members.

### Still open

The duplicate tables themselves are not fixed — they are still extracted, still
carry NULL `dedup_status`, and are now merely classified consistently enough not
to double count. A future ingest of a document with the same page structure can
